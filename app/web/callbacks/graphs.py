...
from app.web.pages.stocks_services import    generate_ytd_performance_figure, generate_historical_valuation_figure, \
    generate_financial_history_figure
...
def register_graph_callbacks(app):
    ...
    def update_all_graphs(store_data, dcf_data):
        try:
            ...
            import traceback
            err_msg = f"Error in Graphs Callback: {str(e)}\n{traceback.format_exc()}"
            ...
            try:
                ...
                logging.error(f"Could not write to error_log.txt: {file_e}")

            # Return error alert for ALL outputs to ensure it's seen
            err_component = dbc.Alert(html.Pre(err_msg), color="danger")
            return err_component, err_component, err_component, err_component, err_component
