...
from app.web.pages.bonds_services import fetch_daily_prices, mock_individual_bond_metrics, generate_yield_history_figure

# [REMOVED] mock data functions - moved to bonds_services.py
...
def register_bonds_callbacks(app: Dash, BOND_METRIC_DEFINITIONS):
    ...
    def render_bond_graph_content(active_tab, data): 
        ...
        if active_tab == "tab-yield-history":
            # [MODIFIED LOGIC] Use Service Function to avoid duplication
            fig = generate_yield_history_figure(tickers, indices) # Pass filtered tickers/indices
            if fig is None:
                 return dbc.Alert("No data available for selected instruments.", color="warning", className="mt-3 text-center")
